...
namespace nta {
    GLTexture ImageLoader::readImage(crstring filePath, GLint minFilt, GLint magFilt, crvec2 dimensions) {
        ...
        if (ilLoadImage(filePath.c_str()) == IL_FALSE) {
            Logger::writeErrorToLog("DevIL failed to load image with error: " + toString(ilGetError()));
        }
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, ilGetInteger(IL_IMAGE_WIDTH), ilGetInteger(IL_IMAGE_HEIGHT), 0,
                     GL_RGBA, GL_UNSIGNED_BYTE, (GLubyte*)ilGetData());
        ...
        Logger::writeToLog("Loaded image (" + toString(ret.width) + " x " + toString(ret.height) + ")");
        return ret;
    }
}
